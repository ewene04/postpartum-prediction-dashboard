            ...
            if missing_columns:
                ...
                if not pd.api.types.is_numeric_dtype(batch_data["MAT_AGE_PU"]):
                    ...
                    if len(predictions) != len(batch_data_numeric):
                        ...
                        with col1:
                            st.markdown(f"<h2 style='text-align: center; font-weight: bold;'>Total Cases</h4>", unsafe_allow_html=True)
                            st.markdown(f"<h4 style='text-align: center; margin-top: -10px;'>{total_cases}</h2>", unsafe_allow_html=True)

                        with col2:
                            st.markdown(f"<h2 style='text-align: center; font-weight: bold;'>Low Risk Cases</h4>", unsafe_allow_html=True)
                            st.markdown(f"<h4 style='text-align: center; margin-top: -10px;'>{low_risk_cases}</h2>", unsafe_allow_html=True)

                        with col3:
                            st.markdown(f"<h2 style='text-align: center; font-weight: bold;'>High Risk Cases</h4>", unsafe_allow_html=True)
                            st.markdown(f"<h4 style='text-align: center; margin-top: -10px;'>{high_risk_cases}</h2>", unsafe_allow_html=True)
